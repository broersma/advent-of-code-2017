...
for program_input in program_inputs:
    if '->' in program_input:
        program_weight, subprograms = program_input.split(' -> ')
        subprograms = subprograms.split(', ')
    else:
        program_weight = program_input
        subprograms = []
    program, weight = program_weight.split()
    program_weights[program] = int(weight[1:-1])
    program_subprograms[program] = subprograms

all_child_programs = []
for program in program_subprograms:
    all_child_programs += program_subprograms[program]
    
all_programs = [program for program in program_weights]

root_program = next(iter(set(all_programs) - set(all_child_programs)))

def get_total_weight(program_name):
    subprogram_weights = {}
    for subprogram in program_subprograms[program_name]:
        subprogram_weights[subprogram] = get_total_weight(subprogram)
    if len(set(subprogram_weights.values())) > 1:
        raise ValueError({subprogram_name: program_weights[subprogram_name] for subprogram_name in subprogram_weights}, subprogram_weights)
    return sum(subprogram_weights.values()) + program_weights[program_name]
    
get_total_weight(root_program)