...
def evaluate(cregister, coperator, cvalue):
    coperators = {
    ...
    return coperators[coperator](cregister, cvalue)
max_value = 5752
for instruction in input:
    print(instruction)
    match = re.match(r"(\w+) (inc|dec) (-?\d+) if (\w+) ([><=!]+) (-?\d+)", instruction)
    register, operator, value, cregister, coperator, cvalue = match.groups()    
    if evaluate(cregister, coperator, int(cvalue)):
        registers[register] += int(value) if operator == 'inc' else -int(value)
